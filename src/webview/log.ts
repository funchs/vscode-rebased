...
function render() {
  ...
  const graphWidth = lanes * LANE_W + LANE_W;
  ...
  rows.forEach((r, i) => {
    ...
    // through-lines: lanes that pass through this row without a commit
    for (const lane of r.active) {
      if (lane === r.lane) continue;
      const tx = lane * LANE_W + LANE_W / 2;
      drawLine(svg, tx, y - ROW_H / 2, tx, y + ROW_H / 2, laneColor(lane));
    }

    // parent links (commit dot -> next row position of each parent)
    if (i + 1 < rows.length) {
      r.parentLanes.forEach((pl) => {
        const px = pl * LANE_W + LANE_W / 2;
        const ny = (i + 1) * ROW_H + ROW_H / 2;
        if (pl === r.lane) {
          drawLine(svg, x, y, px, ny, laneColor(r.lane));
        } else {
          // diagonal merge/branch
          drawPath(svg, x, y, px, ny, laneColor(pl));
        }
      });
    }
...
function drawLine(svg: SVGElement, x1: number, y1: number, x2: number, y2: number, color: string) {
...
function drawPath(svg: SVGElement, x1: number, y1: number, x2: number, y2: number, color: string) {
